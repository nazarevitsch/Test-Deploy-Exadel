...
public class ImagesController {
    ...
    public ResponseEntity<Message> upload(@RequestParam MultipartFile file){
        try {
            ...
            log.debug("File was uploaded to  Amazon. Link:{" + link + "}");
            return new ResponseEntity<>(new Message(link), HttpStatus.OK);
        } catch (Exception e) {
            return new ResponseEntity<>(new Message("Image Storage is temporarily unavailable!"), HttpStatus.NOT_FOUND);
        }
    }
}
